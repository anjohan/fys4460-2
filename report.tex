...
Since each simulation takes a non-trivial amount of time to run, I avoid redoing simulations with the following \texttt{make} rule.
\lstinputlisting[linerange={snippetstart-snippetend}]{{i/Makefile}}
A \texttt{Pool} object from \texttt{multiprocessing} is used to call a Python analysis function for each number of spheres in parallel. This function issues a \texttt{make} command, reads the resulting log file and calculates the permeability from~\vref{eq:meanvel}. The result is shown in~\vref{fig:permeability}.
\begin{figure}[tbh]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Permeability as a function of porosity in a system with spheres making up the matrix. Comparison with the theoretical model in~\vref{eq:modelpermeability} shows that the fit is not particularly good, but at least the trend is similar.}%
